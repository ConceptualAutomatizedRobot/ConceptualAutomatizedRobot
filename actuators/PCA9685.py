...
class PWM(object):
    ...
    def _check_i2c(self):
        ...
        print("I2C bus number is: " + bus_number)
        ...
        output = commands.getoutput(cmd)
        print("Your PCA9685 address is set to " + str(self.address))
    ...
    def frequency(self, freq):
        '''Set PWM frequency'''
        if self._DEBUG:
            print(self._DEBUG_INFO + 'Set frequency to ' + str(freq))
        ...
        prescale_value -= 1.0
        if self._DEBUG:
            print(self._DEBUG_INFO + 'Setting PWM frequency to ' + str(freq) + ' Hz')
            print(self._DEBUG_INFO + 'Estimated pre-scale: ' + str(prescale_value))
        prescale = math.floor(prescale_value + 0.5)
        if self._DEBUG:
            print(self._DEBUG_INFO +'Final pre-scale: ' +str(prescale))
        ...
        time.sleep(0.005)
        self._write_byte_data(self._MODE1, int(old_mode) | 0x80)

    def write(self, channel, on, off):
        '''Set on and off value on specific channel'''
        if self._DEBUG:
            print(self._DEBUG_INFO + 'Set channel "' + str(channel) + '" to value "' + str(off) + '"')
    ...
    def write_all_value(self, on, off):
        ...
        if self._DEBUG:
            print(self._DEBUG_INFO, 'Set all channel to value "' + str(off) + '"')
        self._write_byte_data(self._ALL_LED_ON_L, int(on) & 0xFF)
        self._write_byte_data(self._ALL_LED_ON_H, int(on) >> 8)
